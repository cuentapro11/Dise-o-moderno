        ...
        >
          <div className="block md:hidden px-10">
            <AnimatePresence mode="wait">
              <motion.div
                key={current}
                className="relative rounded-lg overflow-hidden shadow-md w-full h-80"
          ...
          <div className="hidden md:flex gap-4 justify-center items-center px-8">
            <AnimatePresence mode="wait">
              {desktopImages.map((img, i) => (
                <motion.div
                  key={`${current}-${i}`}
                  className={`relative rounded-lg overflow-hidden shadow-md ${i === 1 ? "w-72 h-80" : "w-60 h-64"}`}
                  initial={{ opacity: 0, scale: 0.88, y: i === 1 ? 0 : 20 }}
